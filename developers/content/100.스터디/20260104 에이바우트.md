...
#### 3) rocky linux 10 다운로드
- 다운로드: `Rocky-10.1-x86_64-dvd1.iso` (8.64 GB), <a href="https://rockylinux.org/ko-KR/download" target="_blank">링크</a>
#### 4) vm 생성
- (탐색기) `C:\vm\study` 디렉토리 생성
- (vmware) File → New Virtual Machine
  ...
  - 20 GB → (Finish)
- (vmware) `develop.el10` 선택
  - Memory: 4 GB
  - Processor: 4 (1 / 4)
  - CD/DVD (IDE) → (Use ISO image file) `Rocky-10.1-x86_64-dvd1.iso`

### 1.2 Guest OS 설치: `develop.el10` (Rocky 10)
- `develop.el10` Guest OS 시작
- Rocky Linux 설치 진입 → Locale 선택: `한국어`, `한국` 선택
#### 1) Network & Host Name 설정
- Ethernet(ens160) → `활성화`
- configure →
...
#### 3) Software Selection 설정
- Base Environment → `Server` → (Done)
#### 4) Installation Destination 설정
...
### 1.3 develop.el10: caddy + code-server 구성

- 목적: `설정 파일`, `스크립트 파일` 편집 편의성

#### 1) code-server 구성

##### a. code-server 설치: dnf install
...
##### c. code-server 서비스: systemd 기반 관리

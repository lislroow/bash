...
- 개인 도메인 준비 (가비아 → dlog.my 구매)
- 가비아: `dlog.my` 도메인 → 도메인 관리 → `code.dlog.my 공유기IP` 추가
- 공유기: Port Forwarding 설정 → `외부IP:443 → 내부IP:443`
- vm 호스트: Port Forwarding 설정 → `vm호스트:443 → vm게스트:443`
- vm 게스트: caddy 가상호스트 설정 → (caddy 재시작) → 자동으로 letsencrypt 인증서 발급 시도 → 조건 충족 시 인증서 발급 완료
...
<details>
<summary>caddy: letencrypt 인증서 발급 설정</summary>
